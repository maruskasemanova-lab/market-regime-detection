...
class MomentumFlowStrategy(BaseStrategy):
    ...
    def __init__(
        self,
        min_signed_aggression: float = 0.12,
        min_directional_consistency: float = 0.58,
        min_imbalance: float = 0.05,
        min_sweep_intensity: float = 0.20,
    ...
    def generate_signal(
        ...
        confidence = 100.0 * (
            0.34 * aggression_score
            + 0.26 * consistency_score
            + 0.24 * imbalance_score
            + 0.16 * sweep_score
        )
