...
class MeanReversionStrategy(BaseStrategy):
    ...
    def __init__(
        self,
        entry_deviation_pct: float = 1.5,   # Enter when X% from VWAP
        atr_stop_mult: float = 1.5,          # Stop at N * ATR
        min_confidence: float = 60.0,        # Minimum confidence to signal
    ...
    def generate_signal(
        ...
        if vwap_distance_pct <= -self.entry_deviation_pct:
            # Confirm with RSI
            if rsi_val < 35:
                confidence += 20
            ...
            # Deviation scoring
            if abs(vwap_distance_pct) > self.entry_deviation_pct * 1.5:
                confidence += 15
        ...
        elif vwap_distance_pct >= self.entry_deviation_pct:
            # Confirm with RSI
            if rsi_val > 65:
                confidence += 20
